...
DEFAULT_TEXT = """
**Saya adalah Zaa-Premium yang menjaga Room Chat Ini . Jangan Spam Atau Anda Akan Diblokir Otomatis.**
"""
...
async def permitpm(client, message):
    ...
    if not bacot:
        return await babi.edit(f"**Gunakan Format : `{cmd}pmpermit on or off`.**")
    ...
    if is_approved:
        ...
        try:
            ...
            await client.send_message(
                chat_id,
                f"<b>Menerima Pesan Dari {biji} !!\nTerdeteksi Developer Dari Zaa-Premium.</b>",
                parse_mode=enums.ParseMode.HTML,
            )
        except:
            pass
